...
int mymain(int argc, char **argv)
{
  ...
  char mybase[6];
  char* tempOPT[512];
  memset(tempOPT,0,sizeof(char*)*512);
  sox_globals.tmp_path = lsx_strdup("/sdcard/vtech");
  for(int b=0;b<argc;b++)
  {
    tempOPT[b]=(char*)malloc(strlen(argv[b])+1);
    strcpy(tempOPT[b],argv[b]);
  }
  ...
  parse_options_and_filenames(argc, tempOPT);//argc,argv
